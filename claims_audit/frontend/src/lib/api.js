...
export const api = {
  ...
  linkSync: (sessionId, fileUrl, sheet) =>
    request(`/sessions/${sessionId}/link-sync`, {
      method: "POST",
      body: JSON.stringify({ file_url: fileUrl, sheet: sheet || null }),
    }),
  uploadFile: async (sessionId, file, sheet) => {
    const form = new FormData();
    form.append("file", file);
    if (sheet) form.append("sheet", sheet);
    const resp = await fetch(`${BASE}/sessions/${sessionId}/upload`, {
      method: "POST",
  ...
  getRuleConfig: (sessionId) => request(`/sessions/${sessionId}/flags/config`),
  updateRuleConfig: (sessionId, config) =>
    request(`/sessions/${sessionId}/flags/config`, {
      method: "PUT",
